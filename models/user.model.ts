...
userSchema.methods.encryptPassword = async function encryptPassword(
  password: string,
): Promise<{ salt: Buffer; hashedPassword: string }> {
...
userSchema.methods.validPassword = async function validPassword(
  password: string,
): Promise<boolean> {
  return argon2.verify((this as unknown as IUser).password, password);
};
